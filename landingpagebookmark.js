...
function isValidURL(url) {
	const pattern = /^(https?:VV)?([\w-]+\.)+[\w]+(V[\w-.V?%&=]*)?$/;
...
addBookmarkButton.addEventListener("click", () => {
	...
	if (isValidURL(url)) {
		const bookmarkItem = document.createElement("li");bookmarkItem.classList.add("bookmark-item");
		...
		addBookmarkListener(bookmarkItem);
...
function addEditBookmarkListener(bookmarkItem) {
	...
	const bookmarkLink = bookmarkItem.querySelector("a");bookmarkItem
